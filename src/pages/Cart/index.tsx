...
const Cart: React.FC = () => {
  ...
  function handleFinishShop(total: number, cashback: number) {
    ...
    const purchased = localStorage.getItem("@Pokemericanas: purchased");
    ...
    localStorage.setItem("@Pokemericanas: purchased", JSON.stringify(arrayPurchased));

    localStorage.removeItem('@Pokemericanas: products');
        ...
        <List>
          {
            listOfCart.length === 0 
              ? <EmptyCart>
                  ...
                  <p>
                    Puxa! <br />
                    No momento você não tem naaada no carrinho!<br />
                    Vamos as compras? 
                  </p>
                </EmptyCart>
